...
from .artifacts import ArtifactStore, default_signing_key
from .audit import AuditLogger
from .config import BridgeSettings
from .jobs import JobManager
from .security import check_kill_switch
from .tools.browser import BrowserToolsMixin, BrowserWorker
...
from .tools.vision import VisionToolsMixin
...
class DesktopBridge(
    ...
    def __init__(self, settings: BridgeSettings | None = None) -> None:
        ...
        self.jobs = JobManager(
            self.settings.job_state_path,
            output_limit=self.settings.command_output_bytes,
        )
        signing_key = (
            self.settings.artifact_signing_key
            or self.settings.action_api_key
            or default_signing_key()
        )
        self.artifacts = ArtifactStore(
            self.settings.artifact_path,
            signing_key,
            ttl_seconds=self.settings.artifact_ttl_seconds,
        )
    ...
    def status(self, *, source: str = "local") -> dict[str, Any]:
        ...
        return {
            ...
            "full_access_active": settings.is_full_access,
            "administrator": settings.is_administrator,
            "kill_switch_active": settings.kill_switch_path.exists(),
            "kill_switch_path": str(settings.kill_switch_path),
            ...
            "capabilities": {
                ...
                "recursive_delete": settings.enable_recursive_delete,
                "clipboard": settings.enable_clipboard,
                "window_control": settings.enable_window_control,
                "ui_automation": settings.enable_ui_automation,
                "ocr": settings.enable_ocr,
                "browser": settings.enable_browser,
                "registry": settings.enable_registry,
                "service_control": settings.enable_service_control,
                "package_management": settings.enable_package_management,
                "network_admin": settings.enable_network_admin,
                "scheduled_tasks": settings.enable_scheduled_tasks,
                "power_control": settings.enable_power_control,
            },
        }
    ...
    def system_info(self, *, source: str = "local") -> dict[str, Any]:
        def operation() -> dict[str, Any]:
            ...
            return {
                ...
                "disks": disks,
                "network_interfaces": list(psutil.net_if_addrs()),
            }
